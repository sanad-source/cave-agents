...
class PriorityTaskQueue:
    """Thread-safe priority queue backed by min-heap with delayed scheduling and strict FIFO."""
    ...
    def _promote_due_jobs(self, now: float) -> None:
        """Promote all scheduled jobs that have reached their scheduled_at to ready_queue."""
    ...
    def pop(self, timeout: Optional[float] = None) -> Optional[Job]:
        """Block up to timeout seconds waiting for an eligible job. Return None if timeout expires."""
        with self._cond:
            ...
            while True:
                ...
                wait_time: Optional[float] = remaining_timeout
                if self._scheduled_queue:
                    delay_wait = max(0.0, self._scheduled_queue[0].scheduled_at - time.time())
                    if wait_time is not None:
                        wait_time = min(wait_time, delay_wait)
                    else:
    ...
    def size(self) -> int:
        """Return the count of queued eligible and scheduled jobs."""
        with self._cond:
