"""taskflow executor module implementing WorkerPool."""
...
class WorkerPool:
    """Thread pool worker executing tasks from a PriorityTaskQueue."""
    ...
    def active_workers(self) -> int:
        """Returns the number of workers actively executing a job."""
        with self._active_lock:
            return self._active_workers

    def register_handler(self, name: str, fn: Callable) -> None:
        """Registers a callable handler by name."""
        with self._lock:
            self._handlers[name] = fn

    def add_listener(self, event: str, callback: Callable[[Job], None]) -> None:
        """Adds an event listener callback for on_start, on_success, on_failure, on_retry."""
    ...
    def shutdown(self, wait: bool = True, timeout: Optional[float] = 5.0) -> None:
        """Signals workers to stop and optionally waits for running jobs."""
        with self._lifecycle_lock:
